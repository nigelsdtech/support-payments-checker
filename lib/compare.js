...
function GetDiffs (params, callback) {
  ...
  */
  ...
  for (var i = 0; i < total; i++) {
    ...
    log.debug('Comparison event: %s on %s (%s)', emailPayType, dateFormat(emailDate,'yyyy-mm-dd HH:MM:ss Z'), emailDate)
    ...
    switch (emailPayType) {
      ...
      case "OOH" : {
        ...
        for (var j = 0; j < oohs.length; j++) {
          ...
          log.trace('+--> Comparing with OOH on %s for %s hours', dateFormat(shiftDateToDay,'yyyy-mm-dd HH:MM:ss'), oohHours);

          if (emailDate == oohDate
            && emailHours == oohHours
          ) {
            log.debug('Match found for OOH shift on %s for %s hours', dateFormat(oohDate,'yyyy-mm-dd HH:MM:ss'), oohHours)
            matchFound = true
            emails.splice(i,1)
            i--
            total--
      ...
      case "L2" : {
        ...
        for (var j = 0; j < cals.length; j++) {
          ...
          log.trace('+--> Comparing with Cal on %s (%s) for %s shift', dateFormat(calDate,'yyyy-mm-dd HH:MM:ss Z'), calDate, calPayType)

          if (emailDate == calDate && emailPayType == calPayType) {
            log.debug('Match found for Calendar %s shift on %s', calPayType, dateFormat(calDate,'yyyy-mm-dd HH:MM:ss Z'))
            matchFound = true
            emails.splice(i,1)
            i--
            total--
